...
def test_replay_ce_is_padding_invariant():
    ...
    left = _row0_replay_loss(
        m, [short_d, long_d], [short_t, long_t], "left"
    )  # doc LEFT-padded (the fix)
    right = _row0_replay_loss(
        m, [short_d, long_d], [short_t, long_t], "right"
    )  # doc RIGHT-padded (the bug)
    ...
    assert d_left < 1e-4, (
        f"left-pad must keep the row's loss invariant (the fix): d_left={d_left:.2e}"
    )
...
def test_replay_training_runs_multi_latent():
    ...
    rows = [
        {
            "input_text": f"case {i}: a dispute concerning topic {i % 3}",
            "target_texts": [f"stage A of {i}", f"stage B of {i}"],
            "tag": "emit",
        }
        for i in range(6)
    ]
    rows += [
        {
            "input_text": f"domain fact {i} to keep the backbone fluent and fluent and fluent and fluent",
            "target_texts": [f"restated fact {i}"],
            "tag": "learn",
        }
        for i in range(2)
    ]
    ...
    assert (
        float(((_flat_trainable(m) - before) ** 2).sum()) > 0.0
...
def _all_learn_rows() -> list[dict]:
    return [
        {
            "input_text": f"domain fact {i} to keep the backbone fluent",
