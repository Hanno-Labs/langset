"""GOLDEN test (test-first): a DISTRIBUTION-level negative (LabelNegTerm) must suppress the blunder's probability
...
the time. LabelNegTerm operates on the reserved-digit softmax (dim_lg) DIRECTLY: for each blunder's codeword
...
Run:  .venv/bin/python tests/test_labelneg_reaches_mass.py
"""
...
from langset.strategies import LabelNegTerm, MultiStepCtx  # noqa: E402


class _FakeTrainer:
    """Only the fields LabelNegTerm reads: the PRECOMPUTED padded codeword-index tensor + counts (mirrors what
    Trainer.__init__ builds) + label_plan. The step loop never builds these — __init__ does, once."""

    def __init__(self, label_neg_codes: list[list[tuple]], label_plan: list) -> None:
        ...
        nr = len(label_plan)
        maxk = max((len(rc) for rc in label_neg_codes), default=0)
        n_rows = len(label_neg_codes)
        pad = torch.full((n_rows, maxk, nr), -1, dtype=torch.long)
        counts = torch.zeros(n_rows, dtype=torch.long)
        for i, rc in enumerate(label_neg_codes):
            for j, cw in enumerate(rc):
                pad[i, j] = torch.tensor(cw, dtype=torch.long)
            counts[i] = len(rc)
        self.label_neg_idx = pad
        self.label_neg_n = counts
...
def _build_ctx(model, dev, blunder_code: tuple) -> tuple[MultiStepCtx, torch.Tensor]:
    ...
    args.lam_label_neg = 1.0
    # label_neg_codes is per-row (list of codeword tuples); one blunder on row 0, applied to each valid tick
    codes = [[blunder_code]]
    c = MultiStepCtx(
...
def test_label_neg_suppresses_blunder_mass() -> None:
    """RED until LabelNegTerm exists and correctly minimizes P(blunder) on the reserved-digit softmax. Asserts the
    ...
    contrib = LabelNegTerm().contribute(c)
    assert contrib is not None, (
        "LabelNegTerm returned None (lam_label_neg<=0 / no codes / no label_plan / no dim_lg)"
    )
...
if __name__ == "__main__":
    ...
    for name in ("test_label_neg_suppresses_blunder_mass",):
        try:
            globals()[name]()
            print(f"{name} PASS\n")
