...
def _cot_loss(model, padding_side: str) -> float:
    ...
    rows_seed = [
        "short",
        "a considerably longer seed sentence with many more tokens than the first row",
    ]
    cots = [
        "reason about row 0 step by step and conclude",
        "reason about row 1 step by step and conclude",
    ]
    ...
    rows = [
        {
            "input_text": f"seed {i}",
            "target_texts": ["a", "b"],
            "cot_text": (None if i == 0 else "reason here"),
        }
        for i in range(4)
    ]
...
def test_cot_text_respects_column_mapping() -> None:
    ...
    rows = [
        {"input_text": f"seed {i}", "target_texts": ["a", "b"], "reasoning": f"think about {i}"}
        for i in range(4)
    ]
...
def test_cot_training_runs_and_is_finite() -> None:
    ...
    assert np.isfinite(post).all() and float(np.abs(post).sum()) > 0.0, (
        "CoT training didn't run / went non-finite"
    )
...
def test_cot_column_alone_is_a_noop() -> None:
    ...
    for r in cot_rows:
        r["cot_text"] = (
            "some reasoning that must be ignored when the CoT strategies are not injected"
        )
    base = _run(base_rows)
    with_col = _run(cot_rows)
    assert np.array_equal(base, with_col), (
        "a cot_text column changed default training — it must be inert when not injected"
    )
...
if __name__ == "__main__":
    ...
    for name in (
        "test_cot_loss_is_padding_side_invariant",
